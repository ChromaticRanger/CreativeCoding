...
// -------------------------------------------------------------------------------
// In this sketch the rules will be more complex
//
// - If an infected agent touches a normal agent the normal agent turns infected.
// - If an infected agent touches a safe agent they affect each other
//  - The safe agent becomes a little infected
//  - The infected agent becomes a little bit safe
//  - After 3 touches from a safe agent an infected agent becomes normal
//  - After 3 touches from an infected agent a safe agent becomes normal
// ------------------------------------------------------------------------------
...
const sketch = ({ context, width, height }) => {
  ...
  for (let x = 0; x < 1; x++) {
    ...
    infected_agents.push(new InfectedAgent(a_x, a_y, 'Infected'))
  }
  ...
  for (let x = 0; x < 1; x++) {
    const a_x = random.range(0, width)
    const a_y = random.range(0, height)
    safe_agents.push(new SafeAgent(a_x, a_y, 'Safe'))
  }
  ...
  for (let i = 0; i < 40; i++) {
    ...
    agents.push(new Agent(x, y, 'Normal'))
  }
        ...
        case 'Safe': {
          ...
          for (let j = 0; j < infected_agents.length; j++) {
            ...
            if (dist < saviour.corona) {
              ...
              safe_agents.push(new SafeAgent(infection.pos.x, infection.pos.y, 'Safe'))
        ...
        case 'Infected': {
          ...
          for (let j = 0; j < agents.length; j++) {
            ...
            if (dist < infection.corona) {
              ...
              infected_agents.push(new InfectedAgent(normal.pos.x, normal.pos.y, 'Infected'))
              // remove agent from agents array
              agents.splice(j, 1)
            }
